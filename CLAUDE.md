...
- **A snapshot swap RETIRES the outgoing snapshot; it does not release it** (A-4a). A transaction resolves its active file at its first store, so a read-modify-write transaction whose store starts a new generation swaps snapshots *while holding pointers into the old one* — and after a seal there is no active file, so the next transaction always takes that path. A handle-live cursor does the same on every observed commit. `zsi_snapshot_retire` moves the mappings to the borrower's hold list on the last reference and lets the rest go; `zsi_hold_fini` at the transaction's or cursor's end is the first moment the bytes may be unmapped. **A snapshot that is still SHARED at the swap keeps our reference instead** — dropping it assumes the remaining holder outlives the borrower, and a cursor does not: fetch, open a cursor, store, close the cursor, and the transaction's borrow is unmapped. That half shipped broken until the sqlite integration hit it as "database disk image is malformed". And retire only fires when the snapshot **actually moved**: `zsi_writer_active` refreshes just on the new-generation path, so retiring an unchanged snapshot hands over our reference and takes a fresh one, leaving an extra for the transaction's life — which makes the commit-site fold's `refcount == 2` check fail and quietly demotes every commit to a full rebuild. Reverting either site to `zsi_snapshot_release` is a use-after-unmap **in the caller**, with no library frame in the backtrace — that is how it shipped until 2026-08-14, found by the sqlite integration. Retention is at *mapping* granularity on purpose: a snapshot also holds a descriptor per file, and retaining whole snapshots swaps a dangle for descriptor exhaustion.
